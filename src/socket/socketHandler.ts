...
export const socketHandler = (io: Server, prisma: PrismaClient) => {
  ...
  io.use(async (socket: AuthenticatedSocket, next) => {
    try {
      ...
      if (decoded.role === 'admin') {
        ...
        if (!customer) return next(new Error('Authentication failed'));
  ...
  io.on('connection', (socket: AuthenticatedSocket) => {
        ...
        if (socket.user?.type === 'customer') {
          ...
          if (process.env.OPENAI_API_KEY) {
            ...
            try {
              ...
              const result: any = await completion.json();
              const aiText: string = result?.choices?.[0]?.message?.content?.trim?.() || '';
              if (aiText) {
    ...
    socket.on('user_stopped_typing', (data: { chatId: string }) => {
